  ...
  return (
    <div className=' bg-white rounded-lg h-[260px] py-4 px-8 '>
      ...
      <ResponsiveContainer width="100%" height="80%">
        <BarChart data={data}>
          <CartesianGrid vertical={false} horizontal={true} stroke="#EFF1F3" strokeDasharray="0" />
          <XAxis dataKey="day" tick={{ fontSize: 10 }} interval={0} axisLine={false} tickLine={false} />
          <YAxis tick={{ fontSize: 10 }} domain={[0, 25000]} ticks={[0, 5000, 10000, 15000, 20000, 25000]} axisLine={false} tickLine={false} tickFormatter={(value) => `${value / 1000}k`} />
          <Tooltip />
          <Bar dataKey="online" fill="#0095FF" barSize={10} radius={[2, 2, 2, 2]} color="#7B91B0" />
          <Bar dataKey="offline" fill="#00E096" name="Offline Sales" barSize={10} radius={[2, 2, 2, 2]} color="#7B91B0" />
        </BarChart>
      </ResponsiveContainer>
